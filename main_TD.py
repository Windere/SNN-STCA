...
def getTIDIGITS(train_path, test_path, train_batch_size=1, test_batch_size=1, mode='spike'):
    ...
    train_data_ = sPtn(train_data['ptnTrain'], train_data['train_labels'], train_data['TmaxTrain'], mode='neuron',
                       dt=dt)
    test_data_ = sPtn(test_data['ptnTest'], test_data['test_labels'], test_data['TmaxTest'], mode='neuron', dt=dt)
...
def main():
    ...
    train_ldr, val_ldr = getTIDIGITS(train_path, test_path, train_batch_size=64, test_batch_size=100)
    ...
    layers.append(STCADenseLayer(structure[0], structure[1], spike_fn, w_init_mean, w_init_std, recurrent=True,
...
if __name__ == '__main__':
    ...
    dtype = torch.float
    structure = [num_in, 1024, num_class]
    w_init_std = 0.1
    w_init_mean = 0.
    epoch_num = 60
    load_if = False
